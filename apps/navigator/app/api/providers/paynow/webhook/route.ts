...
export async function POST(request: Request) {
  ...
  const { data: event, error: eventError } = await supabase.rpc("ingest_money_provider_event", {
    p_provider_id: "paynow",
    p_event_id: eventId,
    p_event_type: values.status ?? "STATUS_UPDATE",
    p_provider_reference: values.paynowreference ?? null,
  ...
  if (status === "Paid" || status === "Failed" || status === "Refunded") {
    const { error } = await supabase.rpc("reconcile_money_funding", {
      p_reference: reference,
      p_status: status === "Paid" ? "PAID" : status === "Failed" ? "FAILED" : "REFUNDED",
      p_provider_reference: values.paynowreference ?? null,
      p_metadata: values,
    });
