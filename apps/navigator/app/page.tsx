...
export default function Home() {
  ...
  return (
    <main className="shell">
      <nav className="nav">
        <div className="brand"><span className="brand-mark">M</span><span>Money OS</span></div>
        <span className="status"><i /> Simulation mode</span>
      </nav>

      <section className="hero">
        <p className="eyebrow">FINANCIAL NAVIGATION</p>
        <h1>Tell us what you want your money to do.</h1>
        <p className="subhead">We turn the outcome into a clear plan, compare routes, and explain the trade-offs.</p>

        <div className="composer">
          <textarea
            value={request}
            onChange={(event) => setRequest(event.target.value)}
            placeholder="e.g. I need Mum to receive $300"
            aria-label="Describe what you want your money to do"
          />
          <button disabled={!request.trim()} onClick={() => setRequest(request.trim())}>Build plan <span>→</span></button>
        </div>

        <div className="chips">
          {examples.map((example) => <button key={example} onClick={() => setRequest(example)}>{example}</button>)}
        </div>
      </section>
      ...
      {request && (
        <section className="preview">
          <div>
            ...
            <h2>{action} request detected</h2>
            <p className="request">“{request}”</p>
          </div>
          <div className="preview-grid">
            <div><span>Mode</span><strong>Simulation</strong></div>
            <div><span>Next</span><strong>Quote → Route → Plan</strong></div>
          </div>
        </section>
      )}
      ...
      <footer>Money OS · M0.1 · No real-money execution</footer>
    </main>
  );
}
